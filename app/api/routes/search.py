...
async def semantic_search(
    ...
    try:
        ...
        logger.info(
            f"Semantic search completed: {len(search_results)} results in {processing_time:.3f}s"
        )
        return response
...
async def find_similar_documents(
    ...
    try:
        ...
        filtered_results = [
            result for result in search_results if result.document_id != document_id
        ][:limit]
        ...
        for result in filtered_results:
            ...
            if (
                doc_id not in unique_documents
                or result.similarity_score > unique_documents[doc_id].similarity_score
            ):
...
async def qa_search(
    ...
    try:
        ...
        logger.error(f"Error performing QA search: {str(e)}")
        raise HTTPException(status_code=500, detail="Internal server error") from e


@router.get("/stats")
async def get_search_statistics(vector_service: VectorService = Depends(get_vector_service)):
        ...
        for (doc_id,) in results:
            try:
                ...
                if chunks:
                    ...
                    _ = await nlp_service.generate_embeddings(chunk_texts)
