...
class VectorService:
    ...
    async def _create_tables(self):
        ...
        self.db_conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_chunks_vector_index_id ON chunks(vector_index_id)"
        )
        ...
        try:
            ...
            for i, chunk in enumerate(chunks):
                ...
                self.db_conn.execute(
                    ...
                    (chunk_id, document_id, content, chunk_index, start_position,
                     end_position, vector_index_id)
                    VALUES (?, ?, ?, ?, ?, ?, ?)
                """,
                    [
                        chunk.chunk_id,
            ...
            for similarity, index_id in zip(similarities[0], indices[0]):
                ...
                if doc_metadata:
                    metadata = {
                        ...
                        "upload_timestamp": (
                            doc_metadata[2].isoformat() if doc_metadata[2] else None
                        ),
